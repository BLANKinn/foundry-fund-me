...
contract helper is Script {
    ...
    function getsepoConfig() public pure returns (NetworkConfig memory) {
        //as network config is a struct we need to use memory
    ...
    function anvilConfig() public returns (NetworkConfig memory) {
        ...
        if (activeNw.priceFeed != address(0)) {
            //if we have already deployed the mock we dont have to deploy again in the same run
            return activeNw;
        }
